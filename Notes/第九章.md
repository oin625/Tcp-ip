...
创建套接字的同时会生成 I/O 缓冲。还行，I/O缓冲还没忘。忘了的话回去看看第五章就行了。
...
输出结果和我们预想的不是很相同，缓冲大小的设置需谨慎处理，因此不会完全按照我们的要求进行。我也尝试了设置为零的，但最终也不是零，他有自己的想法。我们只能大致修改。
...
大约三分钟，书上是这么说的，但我等了快十分钟了，它的time-wait状态却还没结束？？？也不知道是不是time-wait是不是还没结束，总之我再次使用这个端口的时候还是会出错。**代填坑**

#### `Time-wait` 状态

观察以下过程：

![9-1](D:\Study_Notes\Tcp-ip\9-1.png)
...
ps。感觉对这个Time-wait状态的具体啥啥的还特别懵的话,参考<a href="">关于三次握手和四次挥手</a>
...
![9-2](D:\Study_Notes\Tcp-ip\9-2.jpg)
...
![9-3](D:\Study_Notes\Tcp-ip\9-3.jpg)
